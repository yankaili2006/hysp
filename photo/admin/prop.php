...
if($step=="copy"){
		...
		$msql->query("delete from {P}_photo_prop where catid='$catid'");
		$p=1;
		$msql->query("select * from {P}_photo_prop where catid='$pid'");
		while($msql->next_record()){
			$propname=$msql->f("propname");
			$xuhao=$msql->f("xuhao");

			$fsql->query("insert into {P}_photo_prop values(
			0,
			'$catid',
			'$propname',
			'$xuhao'
...
if($step=="add"){
	$msql->query("select count(id) from {P}_photo_prop where catid='$catid'");
	if($msql->next_record()){
...
$msql->query("select max(xuhao) from {P}_photo_prop where catid='$catid' ");
if($msql->next_record()){
$max=$msql->f('max(xuhao)');
}

$max=$max+1;
$msql->query("insert into {P}_photo_prop values(
0,
'$catid',
'$propname',
'$max'
...
if($step=="modify"){
$msql->query("update {P}_photo_prop set propname='$propname',xuhao='$xuhao' where id='$id'");
}
if($step=="del"){
$msql->query("delete from {P}_photo_prop where id='$id'");
}
